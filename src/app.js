...
function renderBodyMap() {
  ...
  return `${sectionHead('身体マップ','痛み・治療・手術の場所を、前面と背面から一目で確認できます。')}
  <section class="card body-map-panel">
    <div class="body-map-intro"><div><span class="badge red">BODY MAP</span><h3>気になる部位の四角をタップ</h3><p>部位ごとに状態、日付、病名・症状、病院、治療・手術内容を保存します。✓が付いた部位は登録済みです。</p></div><div class="body-map-totals">${BODY_STATUS_OPTIONS.map(option=>`<div class="status-${option.className}"><b>${totals[option.id]}</b><span>${option.short}</span></div>`).join('')}</div></div>
    ${bodyStatusLegend()}
...
async function registerServiceWorker() {
  ...
  try {
    const registration = await navigator.serviceWorker.register('./sw.js?v=3.6.0', { updateViaCache:'none' });
    await registration.update();
    if (hadController) {
      navigator.serviceWorker.addEventListener('controllerchange',()=>{
        const refreshKey='life-compass-sw-refresh-v3.6.0';
