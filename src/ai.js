...
export function buildContext(state) {
  return {
    ...
    incomeRecords: state.records.filter(x => !x.deletedAt && (x.kind === 'incomeRecord' || x.domain === 'income')).slice(-30),
...
export function buildNotebookMarkdown(state) {
  ...
  const sections = [['目標',state.goals],['習慣',state.habits],['夢・楽しみ（欲しいもの・行きたい場所・やってみたいこと／挑戦／体験）',state.wishes],['健康',state.healthItems],['人生比較',state.comparisons],['人生タイムライン',state.timeline],['商品・事業',state.products],['定期レビュー',state.reviews],['未来シミュレーション',state.simulations],['収入',state.records.filter(x => x.kind === 'incomeRecord' || x.domain === 'income')],['記録',state.records.filter(x => x.domain !== 'income')]];
